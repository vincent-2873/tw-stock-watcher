...
export default async function StockDetail({ params }: { params: Promise<{ code: string }> }) {
  ...
  try {
    ...
    for (const md of monthData.reverse()) {
      if (md.title) {
        // TWSE title 格式:「115年04月 2330 台積電           各日成交資訊」
        // 取出 code + name 兩段,並驗證 code 與 URL 參數一致(避免張冠李戴)
        const m = md.title.match(/\d+年\d+月\s+(\S+)\s+(\S+)\s+各?日?成交資訊/);
        if (m) {
          const tCode = m[1].trim();
          const tName = m[2].trim();
          if (tCode === code) {
            stockName = tName || stockName;
          }
        }
      }
